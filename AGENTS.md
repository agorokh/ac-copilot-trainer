...
- **Own-headset endpoint (#672 Part A):** `AC_COPILOT_AC_AUDIO_DEVICE` (or `settings.json` `ac_audio_device`; env wins) declares the output endpoint **Assetto Corsa / FMOD plays through** — the launcher cannot read AC's device itself. Game Point compares it against the sidecar's live `/health` `voice.device_name` and emits a `voice_endpoint` check row in `status.json` (`shared` / `distinct` / `undeclared` / `unknown`); a `shared` verdict also flips the Voice row to the amber `SHARED_ENDPOINT` state with remediation text. The warning is **advisory only** — every verdict is `ok=True`, so it never blocks START and never reddens the aggregate status, because the rig today ships deliberately pinned to the shared endpoint. Remediate by pointing `AC_COPILOT_VOICE_DEVICE` / `AC_COPILOT_VOICE_HOST_API` at the dedicated headset endpoint. Leave `AC_COPILOT_AC_AUDIO_DEVICE` unset to opt out (the row then reads `undeclared`, never a false-positive warning).
