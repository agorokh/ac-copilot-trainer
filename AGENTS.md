...
- **Stable AC menu prerequisite (#726):** on the supported CSP 0.2.11 runtime, `cfg/extension/gui.ini` must contain `[NEW_UI] REPLACE_MAIN_MENU=0`; New UI ignores `ac.tryToStart`. Game Point checks this before launching Stable AC and reports `menu_config_required` with the exact remediation path. Set non-secret `ac_user_dir` in `settings.json` or `AC_COPILOT_AC_USER_DIR` when standard Documents/OneDrive discovery is insufficient. Start control is explicitly armed by the authenticated harness only; an ordinary trainer-app load never auto-presses Drive.
