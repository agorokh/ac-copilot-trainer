...
- **Voice phrase bank (M0 voice coaching):** `AC_COPILOT_VOICE_BANK` — path to a baked phrase-bank directory; used by `tools.ai_sidecar` and the Game Point launcher when no `--voice-bank` flag is passed. Requires `AC_COPILOT_REFERENCE_ARCHIVE` for live cue anchoring. Optional `AC_COPILOT_ALIEN_LINE` (or launcher `alien_line`) points Coach v2 at a verified same-combo alien-line artifact; it replaces only bounded per-corner speed targets, keeps human-reference brake/throttle/steer technique, and fails closed with an explicit `/health` reason. Activation recomputes provenance from sibling `plant_id/`, hashes the current `fast_lane.ai`, and re-runs corridor/plant-envelope checks; non-standard sources may use `AC_COPILOT_ALIEN_PLANT`, `AC_COPILOT_ALIEN_FAST_LANE`, or `AC_COPILOT_AC_ROOT`. Runtime routing is controlled by `AC_COPILOT_VOICE_BACKEND` (`rtmixer` or `sounddevice`), `AC_COPILOT_VOICE_DEVICE`, `AC_COPILOT_VOICE_HOST_API`, and `AC_COPILOT_VOICE_VERBOSITY`. Optional `AC_COPILOT_VOICE_TTS=1` enables the lightweight pyttsx3 fallback only when no phrase bank is configured; tune it with `AC_COPILOT_VOICE_RATE` (default `240`) and `AC_COPILOT_VOICE_VOLUME` (`0.0`–`1.0`). **Bank precedence (#672 Part B):** in the Game Point launcher `AC_COPILOT_VOICE_BANK` takes precedence over `settings.json` `voice_bank` **whenever it is present** — including set-but-blank, which *clears* the settings value and parks voice. Because the effective value alone cannot tell a parked bank from an unconfigured one, the launcher's Voice row and `status.json` now carry the arm source (`bank armed via env` / `bank cleared via env` / `bank armed via settings`).
- **Own-headset endpoint (#672 Part A):** `AC_COPILOT_AC_AUDIO_DEVICE` (or `settings.json` `ac_audio_device`; env wins) declares the output endpoint **Assetto Corsa / FMOD plays through** — the launcher cannot read AC's device itself. Game Point compares it against the sidecar's live `/health` `voice.device_name` and emits a `voice_endpoint` check row in `status.json` with one of five verdicts: `shared` (same endpoint — the warning), `distinct` (checked, they differ), `undeclared` (`AC_COPILOT_AC_AUDIO_DEVICE` unset, nothing checked), `unknown` (voice is not running, so it holds no endpoint), and `unverifiable` (voice **is** running through a backend that reports no device — the `AC_COPILOT_VOICE_TTS=1` pyttsx3 fallback, which speaks through the Windows *default* endpoint, so the invariant cannot be confirmed). `shared` flips the Voice row to the amber `SHARED_ENDPOINT` state with remediation text; `unverifiable` flips it to amber `ENDPOINT_UNVERIFIED` — an unconfirmable check must not read as an all-clear. Name matching is exact after case/whitespace normalization, plus a one-directional allowance for PortAudio's 31-character **MME** truncation (MME only, and only on the health-reported side — the operator's declaration is never a truncation artifact). The warning is **advisory only** — every verdict is `ok=True`, so it never blocks START and never reddens the aggregate status, because the rig today ships deliberately pinned to the shared endpoint. Remediate by pointing `AC_COPILOT_VOICE_DEVICE` / `AC_COPILOT_VOICE_HOST_API` at the dedicated headset endpoint. Leave `AC_COPILOT_AC_AUDIO_DEVICE` unset to opt out (the row then reads `undeclared`, never a false-positive warning).
...
- **Stable AC prerequisites (#726):** on the supported CSP 0.2.11 runtime, `cfg/extension/gui.ini` must contain `[NEW_UI] REPLACE_MAIN_MENU=0`; New UI ignores `ac.tryToStart`. Game Point checks this before launching Stable AC and reports `menu_config_required` with the exact remediation path. Set non-secret `ac_user_dir` in `settings.json` or `AC_COPILOT_AC_USER_DIR` when standard Documents/OneDrive discovery is insufficient. Stable AC also requires the default `sidecar_port=8765`: the launcher cannot safely rewrite CSP's persisted `wsSidecarUrl` before AC starts, so it reports `sidecar_port_unsupported` rather than starting with a disconnected Lua control peer. Custom ports remain available for sidecar-only operation. Start control is explicitly armed by the authenticated harness only; an ordinary trainer-app load never auto-presses Drive.
...
- Learned rules live in `.claude/rules/learned/` (mirrored in `.cursor/rules/learned/`); this block is no longer auto-updated.
...
<!-- CHANGELOG:START -->
- 2026-07-19: Rig launch reliability (#628/PR #629) — `acpmf_*` shared sections OUTLIVE `acs.exe` (measured PRESENT 14 s after `taskkill`) and stay mapped ~6 s INTO the next `acs.exe`'s lifetime, so a corpse reading is briefly stale-but-plausible. **No `acpmf` field is trustworthy until the packet id ADVANCES — that advance is itself the proof of a live writer, because a corpse is a frozen snapshot that cannot advance on its own.** So `SectionOwnershipGate` consults ONLY the packet stream (do NOT re-add a process-liveness probe around it — that reintroduces the strict-vs-debounced disagreement #628 removed); real death stays `classify()`'s job via its debounced `acs_alive`. `is_live`/`is_in_pit` are exactly as stale as `packet_id` (a corpse reporting READY fired the Car0 handshake before AC existed → 6/6 attempts recorded as `froze`). Before trusting any rig freeze rate, confirm the launcher is on this fix. `QueryThreadCycleTime` gives a ~5 s spin-vs-block read without dumps, but a high cycle count is meaningless unless the graphics packet is simultaneously static — a healthy 67-thread `acs` burns a full core by design. `0x147B` in CSP's `accRenderingAdv.dll` is div-by-100 magic (base-100 bignum), NOT a hash multiplier — #627 §3.5 is wrong.
