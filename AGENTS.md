...
- **Driver profile/progression (#403):** `AC_COPILOT_DRIVER_PROFILE` — optional path to `journal/driver/profile.json`; used by Coach v2 to load returning-driver skill levels and tune cue density. Generate or refresh it with `python -m tools.ai_sidecar.driver_profile --lap-dir <journal/laps>`. When unset, Coach v2 tries `journal/driver/profile.json` under the sidecar's current working directory and otherwise keeps the baseline cue policy.
