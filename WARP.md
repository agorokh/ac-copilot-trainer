...
- **Focus practice mode (issue #44):** In `ac_copilot_trainer.lua` `config`, set **`focusPracticeCornerLabels`** to a comma list such as **`"T1,T2,T3"`** (labels match `corner_analysis` sector tags `T1`, `T2`, …). Leave it **empty** to auto-target the up-to-**`focusPracticeAutoCount`** worst corners from consistency (needs **≥2 laps** with valid telemetry so `consistencySummary` exists). Toggle **Enable (this session)** in the main HUD; the flag **clears when you leave the track** (`resetRuntimeAfterLeavingTrack`). **Rolling session reset** clears auto corner data (`focusWorstThree` / `lastLapCornerFeats`) but **keeps** the checkbox on/off. With focus on, **brake marker walls** for matching corners use a **wider, taller, brighter** shader quad; other markers are **dimmed** when `focusPracticeDimNonFocus` is true and last-lap corner features exist. Spline matching uses a small normalized tolerance (see `focus_practice.brakeSplineMatchesFocus`). **Post-lap coaching text** is filtered to lines that mention the focus labels (or a single fallback line). **Racing line** emphasis is unchanged in this slice (markers + HUD + hints only).
