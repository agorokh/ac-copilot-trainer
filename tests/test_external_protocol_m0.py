...
def test_telemetry_tick_rejects_bad_race_management_channels():
    assert (
        "fuel_l must be >= 0"
        in (validate_inbound(make_telemetry_tick(_full_payload(fuel_l=-1.0))) or "")
    )
    assert (
        "tyre_wear_pct.xx is not a known corner"
        in (
            validate_inbound(make_telemetry_tick(_full_payload(tyre_wear_pct={"xx": 1.0})))
            or ""
        )
    )
    assert (
        "weather_type must be a string"
        in (validate_inbound(make_telemetry_tick(_full_payload(weather_type=7))) or "")
    )
