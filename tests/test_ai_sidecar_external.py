...
def test_validate_inbound_accepts_known_types() -> None:
    assert ep.validate_inbound({"type": "hello", "client": "screen-01"}) is None
    assert ep.validate_inbound({"type": "config.get", "key": "hudEnabled"}) is None
    assert (
        ep.validate_inbound({"type": "config.set", "key": "hudEnabled", "value": True})
        is None
    )
    assert ep.validate_inbound({"type": "action", "name": "toggleFocusPractice"}) is None
    assert (
        ep.validate_inbound({"type": "state.subscribe", "topics": ["lap"]}) is None
    )


def test_validate_inbound_rejects_invalid() -> None:
    assert "non-empty" in (ep.validate_inbound({"type": "hello"}) or "")
    assert "non-empty 'key'" in (ep.validate_inbound({"type": "config.get"}) or "")
    assert "value" in (ep.validate_inbound({"type": "config.set", "key": "k"}) or "")
    assert "unknown action" in (
        ep.validate_inbound({"type": "action", "name": "rmRfRoot"}) or ""
    )
    assert "unknown type" in (ep.validate_inbound({"type": "explode"}) or "")
        ...
        async with _running_sidecar(token="s3cret") as port:
            try:
                async with ws_connect(f"ws://127.0.0.1:{port}/"):
                    return 0
            except websockets.exceptions.InvalidStatus as exc:  # type: ignore[attr-defined]
