...
tree cleanup, source/frozen Game Point dispatch, and Release AC cancellation throughout bounded
startup waits (`b703973`). Required checks are green, all 25 GraphQL threads are resolved, and the
enforce resolve gate is clean. The PR remains open and unmerged; Windows-rig proof is the remaining
separate state. Detail:
