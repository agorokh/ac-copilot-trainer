...
Stable-AC-specific release signaling (`58eba0f`). The final review rounds also validate the
...
them during subsequent watcher samples instead of dropping their owning references; a standalone
probe carries its retained controller on the cleanup exception. Auto-drive similarly carries the
controller on its atomic-abort exception, allowing process teardown to release the mapping and lock
together without killing a healthy AC session. Fatal cleanup
...
pinned by an explicit settings-file test. Repository parity is `3214 passed`, `77 skipped`,
`86.75%` coverage.
