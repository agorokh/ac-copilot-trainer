...
Stable-AC-specific release signaling (`a54db2c`). The final review rounds also validate the
...
the writable section's partial-close safety guard. Fatal cleanup suppresses release before
...
pinned by an explicit settings-file test. Repository parity is `3222 passed`, `77 skipped`,
`86.77%` coverage.
