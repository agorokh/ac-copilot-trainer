...
- **#6 (ac-copilot-trainer, local 2026-03-30):** Telemetry + brake detection + JSON persistence + lap-aware best/last brake sets + ImGui HUD. **Branch:** `feat/issue-6-telemetry-brake-persistence` (create from `main`). **PR:** open Draft early with `Fixes #6`; run `make ci-fast` (GitHub Actions uses Ubuntu + bash policy script). Copy `src/ac_copilot_trainer/` to `assettocorsa/apps/lua/ac_copilot_trainer/` for in-game validation. Next: mark PR ready, then **`pr-resolution-follow-up`** loop (`sleep 600`, GraphQL `reviewThreads`). **`pr-resolution-follow-up` (2026-03-30):** no **open** PR on `agorokh/ac-copilot-trainer` yet — cannot poll checks or `reviewThreads` until **P** exists; open the PR then re-run the loop (or `Task(generalPurpose, …)` with `.claude/agents/pr-resolution-follow-up.md` checklist in Cursor).
