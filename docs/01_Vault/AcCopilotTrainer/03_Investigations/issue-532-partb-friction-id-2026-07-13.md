...
- **Closed-loop full-lap A/B (identified vs generic lap time): NOT completed — rig-gated.** After ~7 launch cycles the rig degraded into compounding **known** flakes: #466 pre-drive overlay stalls, #537 (CM serving cached **Spa** instead of Magione), intermittent `acs.exe` `sim_dead`. Recovery is a fresh rig session / reboot (#466 runbook). Not a plant-model issue. #532 kept OPEN for this final drive.
...
- Closed-loop Magione A/B once the rig is rebooted (blocked by #466 / #537).
