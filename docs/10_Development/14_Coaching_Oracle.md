...
**Verified (2026-06-28, PR #338 `581bd7c`):** GitHub CI green (`build`, `conformance`, `Canonical docs exist`); `pytest tests/test_coaching_oracle.py` 18/18 pass; `ruff format --check` + `ruff check` clean. PR #334 baseline also green (`build`, `conformance`, `pip-audit`, `Canonical docs exist`, `score`, `classify`). Full `make ci-fast` coverage step requires the project venv (`pip install -e '.[dev]'`).
