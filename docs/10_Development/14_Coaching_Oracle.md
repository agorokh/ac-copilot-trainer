...
**Verified (2026-06-28, PR #338 `01ed6b5`):** GitHub CI green (`build`, `conformance`, `Canonical docs exist`); `pytest tests/test_coaching_oracle.py` 17/17 pass; `ruff format --check` + `ruff check` clean. PR #334 baseline also green (`build`, `conformance`, `pip-audit`, `Canonical docs exist`, `score`, `classify`). Full `make ci-fast` coverage step requires the project venv (`pip install -e '.[dev]'`).
