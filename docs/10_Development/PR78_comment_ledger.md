...
| 3115968326 | cursor[bot] | yes |
| 3116030820 | cursor[bot] | yes |
| 3116175068 | cursor[bot] | yes |
| 3116252990 | cursor[bot] | yes |
| 3116252995 | cursor[bot] | yes |
...
### Post-`482baa9` audit (Cursor Bugbot inline)

- **3115968326**: `lap_archive.stats` treats `io.fileSize` results like `rotate`: add only positive sizes; for negative indicators use the same `250 * 1024` heuristic instead of `tonumber(s) or 0` (which subtracted `-1` from the running total).

### Post-`a1a1e26` audit (Cursor Bugbot inline)

- **3116030820**: `lap_archive.write` wraps `f:write(raw)` in `pcall` (same posture as flush/close) so a throwing `write` cannot unwind `script.update`; failures close/remove the partial file and return `write failed: ...`.

### Post-`e562a0f` audit (Cursor Bugbot inline)

- **3116175068**: Lap archive disk-cap slider uses `type(ui.slider) == "function"` (same guard as coaching sliders) instead of `ui.slider ~= nil` so CSP cdata callables stay consistent across Settings controls.

### Post-`e62836a` audit (Cursor Bugbot inline)

- **3116252990**: `lap_archive.stats` mirrors `rotate` scan handling: only `type(name) == "string"` entries increment `count` and participate in size totals.
- **3116252995**: `ws_bridge` sidecar exit callback treats only `exitCode == 0` as clean; any missing/unparseable exit metadata (`codeNum == nil`) follows the nonzero streak/backoff path (`elseif codeNum ~= 0` in Lua).
