...
| `--setup` run: setup `applied=True` but `stage=hijack` | The car stalled at AC's non-hijackable "0 seconds" overlay through every launch cycle (the setup re-bake breaks CM's immediate-start, #466). The setup is applied/verified. The harness fast-fails each stalled cycle and **recycles a fresh launch**; that relaunch is the only recovery with recorded successes (a keypress nudge was verified NOT to dismiss the overlay). If it still exhausts `max_launches`, record the denominator on #627 and hand over the open condition. **Do not borrow the #627 freeze-accumulator reboot advice for this rendering/menu-park shape** — no measured evidence says a reboot makes CSP expose `Car0`. |
...
| `stage=hijack` on EVERY cycle, and `resilient_launch` reports `not_drivable` (before 2026-07-29: `froze`) | AC reached LIVE and is **rendering normally** but parked at AC's pre-drive session menu (Drive/Setup/Exit sidebar, "Practice — 0 seconds"), so CSP never exposes `Car0`. **This is #466, NOT the #627 render wedge** — confirm by reading the packet streams: a wedge PINS `acpmf_graphics.packetId`, whereas this keeps it advancing (measured 2026-07-29: gfx +114/s, physics +374/s, stable ≥66 s, screenshot at `.scratch/issue627/live-after-cold-cm.png`). A reboot does **not** address it and `FORCE_START` is a refuted dead end (above). The human path still works — the operator clicks **Drive** — so a rig in this state serves human coaching sessions while blocking autonomous ones. **Do NOT re-diagnose this as a Content Manager setting** — the operator confirmed CM's "start race immediately" has always been enabled, and the generated preset's `StartType=START` is correct, so neither is the lever. Refuted 2026-07-29 in the same session: CSP's `[TWEAKS] USE_THROTTLE_TO_START` (rig has it at `1`, CSP default `0`) is a plausible-sounding cause and is **also refuted** — flipping it to `0` still gave 3/3 no-`Car0`. Measured that day: **0/6 `auto_drive` invocations** landed a drive (~10-14 launch cycles, 3x hijack / 1x never-LIVE / rest hijack), and the last completed drive on this rig was 2026-07-27 10:56 (`cfg/extension/state/odometers.ini` + `consumption.ini` only write when a car actually drives). Nothing changed in the AC/CSP/CM config trees between that drive and the failures. **Recovery — one recommendation:** re-run and let the fast-fail relaunch recycle land the hijack; that is the only mechanism with recorded successes ("hijack landed on probe 2 or 3", ~4/20 historical stall rate). **Do not reboot for this shape.** The reboot remediation in `resilient_launch`'s own summary text and in the freeze rows below is scoped to the #627 per-boot FREEZE accumulator; there is no measured evidence that a boot changes whether CSP exposes `Car0`, so rebooting here is disruption without a stated expected benefit. If the recycle keeps failing at 0/N as it did on 2026-07-29, treat it as an open condition with cause unknown and record it on **#627** (the master rig brief, and the only OPEN tracker here — #466 is CLOSED, so it names the *shape* historically but is not where new evidence belongs) — record the denominator and hand it over rather than reaching for a remediation borrowed from a different failure mode |
