...
GitHub operations (issues, PRs, API) go through the **`gh` CLI** — not an MCP server. The `github` and `context7` MCP servers are **retired fleet-wide and must not be wired back** (governance-hub#274; conformance CI blocks them). <!-- mcp-retirement-ok --> For third-party library/API questions use official docs and targeted web search. The repo-local `repo-knowledge` server must launch via its wrapper (`bash scripts/mcp/repo-knowledge.sh`) in every harness config — never a bare python module launch. <!-- mcp-retirement-ok --> Add DB/browser tools per project (see [OPTIONAL_CAPABILITIES.md](OPTIONAL_CAPABILITIES.md)).
