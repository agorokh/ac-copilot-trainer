...
- [ ] **Settle CI compute BEFORE relying on CI — the copied lanes are all self-hosted, act by visibility.** An unserved lane **queues forever with no red build** (workstation-ops#2896, observed bootstrapping tuning-atelier). **Private child** → register a runner slot in **workstation-ops**: add `'<github_org>/<repo>': {instances: 1}` to `ops/github-runner/fleet.yaml` (or the host map that owns your lane), add the matching `github-runner-<repo>` row to `ops/service.yaml` (`test_service_catalog_tracks_every_declared_slot` enforces the lockstep), then run reconcile `--apply` on that runner host. **Public child** → do **NOT** attach self-hosted runners (fleet policy `adr-2026-06-27-local-ci-compute-architecture` keeps public repos GitHub-hosted; no self-hosted runners on public fork-PR surfaces): migrate the copied `runs-on` lanes to GitHub-hosted images, or keep the repo private. Defense-in-depth: the hourly registration sweep (workstation-ops#2908) flags a starving private lane within ~90 minutes of its first stale dispatch.
