...
5. the post-merge SessionStart hook (present in full-template spokes; absent in this minimal public spoke) → if local `main` is behind `origin/main`, prints a `POST-MERGE STEWARD NEEDED` routing block and writes `.scratch/post-merge-needed.json`. This does not merge or pull by itself; it makes the agent-facing next action explicit before new implementation work starts.
...
When the orchestrator (`orchestrate`) or any agent invokes `Task(subagent_type=…, prompt=…)`:
...
| Subagent ignores memory contract | Orchestrator did not embed `## Pre-loaded memory context` in the Task prompt | Orchestrator bug — fix in `~/~/.agents/skills/orchestrate/SKILL.md`; the lockfile still gates the subagent's Edit/Write, so blast radius is contained |
