...
| Subagent ignores memory contract | Orchestrator did not embed `## Pre-loaded memory context` in the Task prompt | Orchestrator bug — fix in `.claude/skills/orchestrate/SKILL.md`; the lockfile still gates the subagent's Edit/Write, so blast radius is contained |
