...
5. the post-merge-steward SessionStart hook (present in full-template spokes; absent in this minimal public spoke) → if local `main` is behind `origin/main`, prints a `POST-MERGE STEWARD NEEDED` routing block and writes `.scratch/post-merge-steward-needed.json`. This does not merge or pull by itself; it makes the agent-facing next action explicit before new implementation work starts.
