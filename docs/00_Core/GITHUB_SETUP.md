...
## Claude Code and agents: GitHub access

All GitHub operations from agents and humans go through the **`gh` CLI** — verify with `gh auth status`. Repo harness configs (`.mcp.json`, `.cursor/mcp.json`, `.codex/config.toml`) must **not** declare a GitHub MCP server: the `github` and `context7` MCP servers were retired fleet-wide in June 2026 and conformance CI blocks their reintroduction (governance-hub#274; audit: `scripts/check_mcp_retirement.py` in governance-hub). <!-- mcp-retirement-ok --> Skill **`~/.agents/skills/new-project-setup`** walks through bootstrap including this step.
